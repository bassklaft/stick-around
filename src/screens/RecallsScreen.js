...
// sourceUrl, dateReported, severity, lastValidated. The disclaimer
// banner above the list spells out where these come from in plain
// language.
...
const ENTRIES = [
  {
    ...
    dateReported: "2003-present",
    lastValidated: LAST_UPDATED,
    year: "2003-present",
  },
  {
  ...
  },
  {
    ...
    sourceUrl: "https://www.fda.gov/animal-veterinary/safety-health/recalls-withdrawals",
    sourceLabel: "FDA recalls page · class action settled 2021",
    dateReported: "2019",
    lastValidated: LAST_UPDATED,
    year: "2019",
  },
...
const SOURCE_TAG = {
  "fda":              { label: "FDA",                        tint: "#9C2A0F" },
  "fda+class-action": { label: "FDA + class action",          tint: "#7A2A14" },
  "advocacy":         { label: "Consumer advocacy",           tint: "#5A4A30" },
  "class-action":     { label: "Active class action",         tint: "#7A4F0A" },
  "journal":          { label: "Peer-reviewed journal",       tint: "#3F5A30" },
};
    ...
    >
      <View style={s.intro}>
        ...
        <Text style={s.introBody}>
          Recalls and consumer alerts shown here are sourced from regulatory bodies (FDA), consumer advocacy aggregators (DogFoodAdvisor, TruthAboutPetFood), and active class action lawsuits. These are NOT FloofLife's opinions — we surface what these sources have published. Active class action allegations are clearly labeled as such and not as proof of harm.
        </Text>
        <Text style={[s.introBody, { marginTop: 8 }]}>
          Real-time top veterinary medical journal integration coming soon — JAVMA, JFMS, and JSAP partnerships in development.
        </Text>
        <Text style={[s.introBody, { marginTop: 8, fontWeight: "700" }]}>
      ...
      {filtered.map((e, i) => {
        ...
        const srcTag = SOURCE_TAG[e.source] || null;
        return (
          <View key={i} style={s.card}>
            ...
            <View style={s.metaRow}>
              ...
              {srcTag && <Text style={[s.sourceTag, { color: srcTag.tint }]}>SOURCE · {srcTag.label.toUpperCase()}</Text>}
            </View>
...
const s = StyleSheet.create({
  ...
  sourceTag:   { fontSize: 9, fontWeight: "800", letterSpacing: 0.8 },
